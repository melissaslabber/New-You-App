...
export default async function handler(req, res) {
  ...
  const prompt = `Read these photographs of one packaged food product sold in South Africa. One image may show the front of the product and another its nutrition label. Extract the English product name, brand, barcode digits, stated serving size, and nutrition per 100 g or per 100 ml. If the label only gives nutrition per serving, calculate the per-100 values using the stated serving size. Return both kilojoules and kilocalories. Convert using kcal = kJ / 4.184 if only kJ is shown, or kJ = kcal * 4.184 if only kcal is shown. Do not guess unreadable values. Return only valid JSON in this exact form: {"name":"","brand":"","barcode":"","servingSize":100,"unit":"g","kj":0,"cal":0,"protein":0,"carb":0,"fat":0,"confidence":"high|medium|low","notes":""}. Use unit g or ml only. Numbers must be ordinary decimals.`;
  ...
  try {
    ...
    const cal = cleanNumber(parsed.cal || (Number(parsed.kj) / 4.184));
    const kj = cleanNumber(parsed.kj || (cal * 4.184));
    return res.status(200).json({ product: { name: String(parsed.name || "").slice(0, 120), brand: String(parsed.brand || "").slice(0, 80), barcode: String(parsed.barcode || "").replace(/\D/g, "").slice(0, 32), servingSize: cleanNumber(parsed.servingSize) || 100, unit: parsed.unit === "ml" ? "ml" : "g", kj, cal, protein: cleanNumber(parsed.protein), carb: cleanNumber(parsed.carb), fat: cleanNumber(parsed.fat), confidence: ["high", "medium", "low"].includes(parsed.confidence) ? parsed.confidence : "low", notes: String(parsed.notes || "").slice(0, 200) } });
  } catch (error) {
