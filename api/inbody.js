...
export default async function handler(req, res) {
  ...
  try {
    const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-flash-lite-latest:generateContent?key=${encodeURIComponent(apiKey)}`, {
      ...
      body: JSON.stringify({ contents: [{ role: "user", parts: [{ text: prompt }, { inlineData: { mimeType: "application/pdf", data: pdf.data } }] }], generationConfig: { temperature: 0.1, maxOutputTokens: 900, responseMimeType: "application/json" } }),
    });
    ...
    return res.status(error?.name === "AbortError" ? 504 : 422).json({ error: error?.name === "AbortError" ? "Reading the report took too long. Please try again." : "The PDF could not be read. Please upload the original InBody PDF." });
  } finally {
